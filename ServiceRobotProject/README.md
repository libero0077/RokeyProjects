...
**서비스(음식배달) 로봇 및 관제 시스템 개발** 프로젝트는 현대 음식점의 워크플로우를 최적화하기 위해 주문 관리, 주방 운영, 음식 배달을 자동화한 통합 솔루션입니다. ROS (Robot Operating System)를 활용하여 구성 요소 간의 원활한 통신을 구현함으로써, 첨단 기술과 세심한 설계를 통해 효율성, 정확성 및 고객 만족도를 향상시킵니다.

## 주요 기능

- **테이블 오더 관리:** PyQt를 사용한 GUI를 통해 9개의 테이블에서 주문을 손쉽게 입력하고 관리할 수 있습니다.
- **주방 디스플레이:** 실시간으로 접수된 주문을 확인하고, 서빙 로봇을 통해 음식을 배달할 수 있습니다.
- **서빙 로봇 제어:** ROS2와 NAV2를 사용하여 서빙 로봇을 시뮬레이션하고, 지정된 테이블로 정확하게 음식을 배달합니다.
...
- **파일:** `kitchen_monitoring.py`, `menu_order.py`
...
1. **저장소 클론:**
    ...
    ```bash
    git clone https://github.com/Y6HYUK/RestaurantServingSystem.git
    cd RestaurantServingSystem
    ```
