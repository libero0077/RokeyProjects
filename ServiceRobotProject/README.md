...
1. **ROS2 노드 실행:**
    ...
    - **nav2를 사용하여 가상 시뮬레이션에서 터틀봇을 움직이기 위해 navigation 런치파일 실행:**

        ```bash
        ros2 launch turtlebot3_navigation2 navigation2.launch.py use_sim_time:=True map:='map.yaml 경로 설정'
        ```

    - **테이블 오더에 메뉴판을 업데이트 해주기 위해서 서비스 서버인 주방 모니터링 노드를 실행:**

        ```bash
        ros2 run menu_order_project test_S
        ```

    - **테이블 오더 노드 실행:**

        ```bash
        ros2 run menu_order_project test_P
        ```

    - **로봇 컨트롤러 노드 실행:**

        ```bash
        ros2 run menu_order_project test_R
        ```

    - **로봇 모니터링 파일 실행:**

        ```bash
        python3 test_robot_monitoring.py
        ```
...
- 류재준 [libero0077](https://github.com/libero0077) : 전체 시스템 통합, 통계 시스템 개발, 데이터베이스 설계 
- 양준혁 [Y6HYUK](https://github.com/Y6HYUK) : 주방 모니터링 GUI 개발, 로봇 컨트롤러 Node 개발, 로봇 모니터링 GUI 개발
- 이상우 [leesw1357](https://github.com/leesw1357) : 테이블 오더 GUI 개발
- 김주원 [juwon407](https://github.com/juwon407) : 전체 시스템 검토 및 발표자료 제작 (예비군 훈련 참여) 
