...
def main(args=None):
    ...
    prepare_joint = [20.38, 38.99, 64.26, -1.10, 77.96, -72.49]
    stack_base_pos = [9.33, 37.95, 55.37, -0.12, 86.79, -82.92]
    
    BLOCK_TRUE_Z = 19
    BLOCK_FALSE_Z = 14

    # 준비 위치, 접근 위치, 적재 위치 정의 (실제 위치 값으로 수정 필요)
    PREPARE_POS = posx([569.58, 212.99, 40, 58.72, -179.99, -34.49 + 90])  # 예시 좌표
    ...
    def initialize_arm(node):
        ...
        movej(INITIAL_JOINT, vel=VELOCITY_F, acc=ACCELERATION_F)
        time.sleep(1)
    ...
    def pick_block(node):
        ...
        while not block_gripped:
            ...
            if check_force_condition(DR_AXIS_Z, max=5):
                ...
                movel(posx([0, 0, block_z * 2, 0, 0, 0]), vel=VELOCITY_F, acc=ACCELERATION_F, ref=DR_TOOL)
                time.sleep(1)
                node.get_logger().info("그리퍼 닫기.")
                grip(node)
                time.sleep(0.5)
    ...
    def place_block(node, layer, stack_base_pos, current_z_values):
        ...
        movel(posx([0, 0, 15, 0, 0, 0]), vel=VELOCITY_F, acc=ACCELERATION_F, ref=DR_TOOL)
        time.sleep(0.5)
        # Force control
        task_compliance_ctrl(stx=[500, 500, 500, 100, 100, 100])
        set_desired_force(fd=[0, 0, -10, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=DR_FC_MOD_REL)
