...
class RobotArmNode(Node):
    ...
    async def execute_callback(self, goal_handle):
        ...
        feedback.status = f"Starting task {task}."  # status 필드 수정
        ...
        for step in ["moving", "gripping box", "lifting box", "placing on conveyor"]:
            if goal_handle.is_cancel_requested:
                ...
                return TaskAction.Result(result=f"Task {task} canceled.")  # result 필드 수정
            
            feedback.status = f"{task}: {step}"  # status 필드 수정
            goal_handle.publish_feedback(feedback)
            self.get_logger().info(feedback.status)
            await self.sleep(1)  # 단계별 실행 시간
            
        self.get_logger().info(f"Task {task} completed.")
        goal_handle.succeed()
        return TaskAction.Result(result=f"Task {task} completed.")  # result 필드 수정
