...
class TestNode(Node):
    def __init__(self):
        ...
        self.camera_connected = True  # 카메라 연결 상태 플래그
        ...
        self.log_publisher = self.create_publisher(String, 'log_mesg', 10)  # log_mesg 퍼블리셔 추가
        ...
        self.camera_connected = self.camera.isOpened() if self.camera else False

        # 카메라 상태 확인 타이머
        self.camera_check_timer = self.create_timer(2.0, self.check_camera_status)  # 1초마다 카메라 상태 점검
    ...
    def check_camera_status(self):
        """카메라 상태를 확인하고 로그 메시지를 전송"""
        print("test 11111111111111111111")
        if self.camera is None or not self.camera.isOpened():
            if self.camera_connected:  # 이전에는 연결되었지만 현재는 끊어졌을 때
                self.get_logger().error("Camera disconnected.")
                self.publish_log_message("Camera disconnected.")
            ...
            if self.camera.isOpened():
                ...
                self.publish_log_message("Camera reconnected.")
    ...
    def publish_log_message(self, log_message):
        """log_mesg 토픽 퍼블리시"""
        msg = String()
        msg.data = log_message
        self.log_publisher.publish(msg)
        self.get_logger().info(f"Published log message: {log_message}")
...
class TestApp(QtWidgets.QWidget):
    def __init__(self, node):
        ...
        # UI 초기화
        self.node.textbox, self.camera_label, self.emergency_textbox, self.log_textbox = self.init_ui()
    ...
    def init_ui(self):
        ...
        # 로그 메시지 텍스트 박스
        log_textbox = QLineEdit(self)
        log_textbox.setPlaceholderText("Log 메시지 입력")
        layout.addWidget(QLabel("Log Messages:"))
        layout.addWidget(log_textbox)

        # 로그 메시지 전송 버튼
        send_log_button = QPushButton("Log 메시지 전송", self)
        send_log_button.clicked.connect(lambda: self.node.publish_log_message(log_textbox.text()))
        layout.addWidget(send_log_button)
        ...
        return textbox, camera_label, emergency_textbox, log_textbox
