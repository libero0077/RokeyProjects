...
class Turtlebot3ManipulationTest(Node):
    def __init__(self):
        ...
        self.box_timestamp = {}        # label_id -> timestamp
            ...
            with self.detected_boxes_lock:
                for det in detections:
                    ...
                    if color not in self.detected_boxes:
                        self.detected_boxes[color] = []

                    # 동일 좌표 박스 중복 추가 방지
                    if not any(math.isclose(box['center_x'], center_x, abs_tol=5) and 
                            math.isclose(box['center_y'], center_y, abs_tol=5) 
                            for box in self.detected_boxes[color]):
                        self.detected_boxes[color].append({
                            'bbox': bbox,
                            'center_x': center_x,
                            'center_y': center_y,
                        ...
                        self.get_logger().info(f"Added new box: {self.detected_boxes[color][-1]}")
    ...
    def get_boxes_by_color(self, color):
        """특정 색상의 박스를 반환."""
        with self.detected_boxes_lock:
            return [box for box in self.detected_boxes.get(color, []) if not box['picked']]
    ...
    def get_available_boxes_by_color(self, color):
        ...
        with self.detected_boxes_lock:
            available_boxes = [box for box in self.detected_boxes.get(color, []) if not box['picked']]
