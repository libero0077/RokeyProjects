...
- AMR노드 실행 영상 : https://youtube.com/shorts/7qBMcnGgQtE
- GUI 실행 영상 : https://youtu.be/0IBQ3xDhxgQ
- AMR 복귀 영상
...
- **주요 기능:**
  ...
  - 객체 추적 및 상태 관리: 탐지된 객체에 고유 ID를 할당하고 추적을 진행, 객체의 상태(내부, 통과, 도주 등)를 관리
...
3. **추적차량 분류:**
   ...
   - 이는 오탐지를 최소화하고 정확한 추적 대상 식별을 가능하게 합니다.
...
6. **추적 종료 판단:**
   - 추적 종료는 모니터링 인터페이스를 통해 사용자가 수동으로 추적 종료를 할 수 있습니다.
   - 또한 객체가 2초 이상 탐지되지 않으면 해당 객체의 추적을 종료합니다.
   - 화면 외부로 차량이 이동하여 ID가 소실되는 경우:
   1. 시스템은 해당 차량의 마지막 위치를 last_position으로 기록합니다.
   2. 해당 객체의 상태를 "Directionless"로 변경하고, 마지막 위치에 고정 크기의 박스를 표시합니다
   3. 해당 차량이 화면에 재진입할 경우, 운영자는 마우스 클릭을 통해 해당 차량에 이전 ID를 수동으로 재할당할 수 있습니다.
