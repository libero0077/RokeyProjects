...
# ROS2 로직 관리 클래스
class GuiNode(Node):
    def __init__(self, robot_status_label=None, media_label=None):
        super().__init__('gui_node')

        # 퍼블리셔 생성
        self.conveyor_publisher = self.create_publisher(String, 'conveyor_state', 10)   # 컨베이어 상태 퍼블리셔
        self.command_publisher = self.create_publisher(String, 'conveyor_command', 10)   # 컨베이어 조작 퍼블리셔
        ...
        # USB Disconnection Subscriber
        self.usb_disconnect_subscription = self.create_subscription(
            String,
            '/usb_disconnect',
            self.usb_disconnect_callback,
            10
        )
    ...
    def usb_disconnect_callback(self, msg):
        """Handle USB disconnection messages."""
        self.get_logger().warn(f"USB Disconnected: {msg.data}")
        # 추가로 GUI에 상태 업데이트
        if hasattr(self, "robot_status_label") and self.robot_status_label is not None:
            self.robot_status_label.setText("USB Disconnected")
...
#------------------------------컨베이어 수동 조작 관련 시작
    ...
    def handle_conveyor_control_radio(self):
        ...
        if self.ui.conveyor_control_radio.isChecked():  # 컨베이어 수동 조작 버튼이 클릭되면
            ...
            if msg_box.exec_() == QMessageBox.Yes:  # Yes를 누르면
                ...
                ############## Start 토픽 발행
                start_msg = String()
                start_msg.data = "start:1000"
                self.gui_node.command_publisher.publish(start_msg)
                self.gui_node.get_logger().info(f"Published: {start_msg}")
            else:
    ...
    def handle_stop_conveyor(self):
        ...
        # Stop 토픽 발행
        stop_msg = String()
        stop_msg.data = "start:1"
        self.gui_node.command_publisher.publish(stop_msg)
        self.gui_node.get_logger().info("Published: stop")
